...
- Agent 2's duplicate check runs against the real seeded trial projects (seeded once per server session via the same guarded `_ensure_seeded()` pattern item §7.2's change-management demo already used, so it never resets a project the session already touched live). Trial data was later curated from 100 down to 20 — see TECH-SPEC.md §14 — after this pass was built; this pass's behavior is unaffected either way.
