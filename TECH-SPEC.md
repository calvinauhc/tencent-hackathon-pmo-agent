...
|---|---|---|---|---|---|---|
| A1 Intake Parser | Haiku 4.5 | Fast | Single call, minimal context (just the submitted form) | Small, fixed schema output | Structured output validated against Postgres columns (§3); low-confidence → Manual Gate, never guessed (§10) | `audit_log.duration_ms`; target < 5s (§8.3) |
| A2 Duplicate Checker | — (embedding model, not chat); Sonnet 5 for the borderline band only | — (embedding); Primary for borderline | Embedding compare is near-instant; LLM adjudication only fires on the borderline band, not every submission | LLM call skipped entirely outside the borderline band — the single biggest token saving in the pipeline | Fixed thresholds (§4); adjudication uses low temperature, structured same/different verdict | `audit_log` tracks borderline-path frequency — signal for retuning §4 thresholds over time |
