...
7. **"Send a project update"** (added post-brief, §14's update log) — "the list is an interactive database": pick any real accepted/in_progress project from a dropdown (not a hardcoded demo project) and submit a typed update email — same real pipeline as Case 8/9 (Agent 11 capture → Agent 12 evaluate → auto-apply or Manual Gate 3), just genuinely parsed from typed text via Agent 11's own deterministic parser (`src/agents/agent11_update_logger.py`'s `parse_update_email()`) rather than a canned payload — the same "never guess a field that isn't actually stated" convention Agent 1's fallback parser (§ "comparing Foo's repo" upversion) already established.
