...
5. The trigger itself is framed as a real submitter action (§14's dated update, "Case 10"): the project originator emails PMO that the project is done and asks for closeout, the same "update from the originator" shape Case 8/9 already use for Agent 11/12 — not PMO or an agent unilaterally deciding to complete something. Once composed and published, `opl_published()` (§11) replies to that SAME originator confirming the OPL exists and giving them the real link to it, so the "what worked / what to reuse" distillation (point 2 above) doesn't just sit in `kb_documents` unannounced — the person who did the work gets told what was captured from it.
